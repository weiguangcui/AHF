...
#ifdef METALHACK  /* Weiguang: these only read gadget format 2!! try to load the hdf5 files with under functions */
                  ...
                  hid_t hdf5_grp[])
{
  ...
  for (type = 0; type < 6; type ++)
  {
    if ((type == 0 || type == 4) && f->header->np[type] != 0) {
      if (f->header->flagdoubleprecision)
      {
        hdf5_datatype = H5Tcopy(H5T_NATIVE_DOUBLE);
        num_bytes = f->header->np[type] * sizeof(double) * 11;
        CommBuffer = (double *)malloc(num_bytes);
      } else {
        hdf5_datatype = H5Tcopy(H5T_NATIVE_FLOAT);
        num_bytes = f->header->np[type] * sizeof(float) * 11;
        CommBuffer = (float *)malloc(num_bytes);
      }
      ...
      io_util_readhdf5(log, f, "Metallicity", type, 1, hdf5_datatype, hdf5_grp, CommBuffer);

      /* Loop over the particles */
      for (i = 0; i < f->header->np[type] * 11; i+=11)
      {
        if (type == 0 || type == 4)
        {
          if (f->header->flagdoubleprecision)
        ...
        if (strg.bytes_float != sizeof(float))
        {
          *((double *)strg.z.val) = fu;
        } else {
          *((float *)strg.z.val) = (float)fu;
        }
                  ...
                  hid_t hdf5_grp[])
{
  ...
  for (type = 0; type < 6; type ++)
  {
    if ((type == 5 || type == 4) && f->header->np[type] != 0) {
      if (f->header->flagdoubleprecision)
      {
        hdf5_datatype = H5Tcopy(H5T_NATIVE_DOUBLE);
        num_bytes = f->header->np[type] * sizeof(double);
      ...
      for (i = 0; i < f->header->np[type]; i++)
      {
        ...
        if (strg.bytes_float != sizeof(float))
        {
          *((double *)strg.age.val) = fu;
        } else {
          *((float *)strg.age.val) = (float)fu;
        }
